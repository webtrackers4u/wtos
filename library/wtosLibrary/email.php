...
class email
{
    ...
    public function From($from, $name=false)
    {
        ...
        if ($name && is_string($name)) {
            ...
            if ($name!=" ") {
                $this->mailfromname="\"".addslashes($name)."\"";
            } else {
                $this->mailfromname=$this->mailfrom;
            }
    ...
    public function Message($var="", $charset="iso-8859-1", $encoding="quoted-printable")
    {
    ...
    public function Attachment($file, $filename=false, $maxsize=false, $disp="attachment", $type=false)
    {
        ...
        if (is_readable($file)) { // get a local file
            ...
            if ($maxsize && $maxsize<filesize($file)) {
                ...
                if ($type) {
                    $this->attfile['type'][]=$type;
                } else {
                    $this->attfile['type'][]="application/octet-stream";
                }
                ...
                if ($disp!="inline") {
                    $this->attfile['disp'][]="attachment";
                } else {
                    $this->attfile['disp'][]=$disp;
                }
    ...
    public function Receipt($to=false)
    {
    ...
    public $mailvers = "mails PHP";

    public $knownmail="local@localhost.com";

    public $checkaddr=false;

    public $checkserv=false;
    ...
    public $mailfromname=false;

    public $mailreply=false;

    public $mailcc=false;

    public $mailbcc=false;

    public $mailreceipt=false;

    public $mailpriority=false;

    public $mailsubj="";

    public $mailbody="";

    public $mailheaders=false;

    public $attfile=false;
    ...
    public function mkCheckServer($email)
    {
    ...
    public function mkHeaders()
    {
        ...
        if ($this->mailvers) {
            $this->headers["X-Mailer"] = $this->mailvers;
        }
    ...
    public function sendMail($to, $from, $fromName, $subj, $body, $attachments='')
    {
        ...
        */
        ...
        if(is_array($attachments)) {
            ...
            $this->Attachment($attachments['file'], $attachments['filename'], $attachments['maxsize'], $disp="attachment", $attachments['type']);
    ...
    public function sendMailAttached($to, $from, $fromName, $subj, $body, $filename)
    {
        ...
        $this->Attachment($filename, $filename, 999999, $disp="attachment", 'pdf');
