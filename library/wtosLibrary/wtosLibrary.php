...
class wtosLibrary extends wtosBase
{
    ...
    function UploadPhoto($name, $path)
    {
        ...
        if (isset($_FILES[$name]['name'])) {
            if (preg_match("/\.(php|js)$/", $_FILES[$name]['name'], $ext)) {
                return false;
            }
...
?>
    ...
    function showDate($dbDate, $format = '')
    {
        ...
        if (trim($dbDate) == '' || $dbDate == '0000-00-00 00:00:00' || $dbDate == '0000-00-00') {
            return '';
        }
        if ($format == '') {
            $format = $this->dateFormat;
    ...
    {
        if ($this->currentPage == 'settings.php') {
            return;
        }
        ...
        if ($exitSystem) { ?>
            ...
            <div class="box" style="margin:auto; margin-top:100px; font-size:22px;">
                ...
                <font style="font-size:12px;"> For acivation code contact <font style="color:#0000FF; font-size:14px;">admin@webtrackers.co.in </font>or <font style="color:#0000FF;font-size:14px;">033 6501 0194</font>
                </font>
            </div>
    ...
    function editSelect($statusArr, $selected, $table, $editFld, $idFld, $idVal, $inputNameID = 'editSelect', $extraParams = 'class="editSelect" ', $colorStatus = array(), $ajaxPage = '', $ajaxMethod = 'POST', $phpFunc = '', $javascriptFunc = '', $advanceOption = '')
    {
        ...
        global $site;
        if ($ajaxPage == '') {
            $ajaxPage = 'wtos/wtosSystemAjax.php';
        }
        $ajaxPage = $site['url'] . $ajaxPage;
        if (is_array($advanceOption)) {
            $advanceOption = implode('--', $advanceOption);
        }
