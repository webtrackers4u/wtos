...
function createFields($dataTosave)
{
    ...
    if(trim($table)!='' && trim($field)!='') {
        ...
        if($Type=='text') {
            $q .=" `$field`  VARCHAR( 100 ) NOT NULL ;";
        }
        if($Type=='Textarea') {
            $q .=" `$field`  TEXT  NOT NULL ;";
        }
        //	if($Type=='Date'){     $q .=" `$field`  DATE NOT NULL ;";   }
        if($Type=='Date') {
            $q .=" `$field`  datetime NOT NULL ;";
        }
        if($Type=='Selectbox') {
            $q .=" `$field`  VARCHAR( 50 ) NOT NULL ;";
        }
        if($Type=='Image') {
            $q .=" `$field`  VARCHAR( 200 ) NOT NULL ;";
        }
        if($Type=='CheckBox') {
            $q .=" `$field`  VARCHAR( 2 ) NOT NULL ;";
        }
        // echo  $q;
        $os->mq($q);
...
function createPagesAjax($data)
{
    ...
    $pluginRoot=DIR_PLUGIN;
    $previewPath=URL_PLUGIN.$viewPage ;
    if(trim($pluginName)!='') {
        $pluginRoot=DIR_PLUGIN.$pluginName.'/';
        $previewPath=URL_PLUGIN.$pluginName.'/'.$viewPage ;
    }
    ...
    $viewPageTemplateLoc=DIR_PLUGIN.'wtosFactory/'.'wtosFactory.DataView.html';
    $ajaxPageTemplateLoc=DIR_PLUGIN.'wtosFactory/'.'wtosFactory.DataAjax.html';
    $pluginMetaTemplateLoc=DIR_PLUGIN.'wtosFactory/'.'wtosFactory.metaData.html';

    $handle = fopen($viewPageTemplateLoc, "r");
    $os->viewPageStrTmpl =(string) fread($handle, filesize($viewPageTemplateLoc));
    fclose($handle);
    $handle = fopen($ajaxPageTemplateLoc, "r");
    $os->ajaxPageStrTmpl =(string) fread($handle, filesize($ajaxPageTemplateLoc));
    fclose($handle);
   ...
   '.$v_and.'=$os->DateQ(\''.$f.'\',$f_'.$v_search.',$t_'.$v_search.',$sTime=\'00:00:00\',$eTime=\'59:59:59\');'."\n";
    ...
    if (!file_exists(DIR_PLUGIN.$pluginName)) {
        mkdir(DIR_PLUGIN.$pluginName, 0777, true);
    }
    ...
    if($processZip=='ok') {
        ...
        if($pluginName!='' && $pluginName!='wtosFactory') {
            ...
            $zipFolder=DIR_PLUGIN.$pluginName;
            $zipFile=BASE_DIR.$pluginName.'.zip';
...
function createPages($data)
{
    ...
    $pluginRoot=DIR_PLUGIN;
    $previewPath=URL_PLUGIN.$ajaxPage ;
    if(trim($pluginName)!='') {
        $pluginRoot=DIR_PLUGIN.$pluginName.'/';
        $previewPath=URL_PLUGIN.$pluginName.'/'.$ajaxPage ;
    }
    ...
    $viewPageTemplateLoc=DIR_PLUGIN.'wtosFactory/'.'wtosFactory.normal.DataEdit.html';
    $ajaxPageTemplateLoc=DIR_PLUGIN.'wtosFactory/'.'wtosFactory.normal.DataList.html';
