...
function showDataAndForm($functionId)
{
    ...
    if($dataRS) {
        ...
        ?>
			...
			<div class="sasclear"> </div>
			<table class="SAAListTable" cellpadding="0" cellspacing="0"><?php

        while($row=$os->mfa($dataRS)) {
            ?> <tr onclick="saa.assignValue('<?php echo $functionId; ?>','<?php echo $row[$returnField] ?>')"> 
			 <td>  <?php echo $cc ?>   </td>
			<?php foreach($fieldsA as $field) { ?>
			 <td>  <?php echo $row[trim($field)] ?>   </td>
			  <?php } ?>
...
<a href="javascript:void(0)" onclick="saa.prepare('<?php echo $functionId; ?>')">S</a>
</span>

<div id="<?php echo $popupBoxId ?>" class="SAA_popupBOX" style="display:none;">
<div class="SAApopoupClose" onclick="saa.popUpClose('<?php echo $popupBoxId ?>')">X</div>
...
<tr> 
			 <td> <input type="text" id="<?php echo $searchTxtId ?>" class="SAASearchTextBox class_<?php echo $searchTxtId; ?>" />    </td>
	 
	
 
	<td><input type="button" style="cursor:pointer;" value="Search" 
 
onclick="saa.prepare('<?php echo $functionId; ?>')" /> </td>
	</tr>
	</table>
</div>
<div class="SaaTabClass" id="SaaTab2" style="display:none;">

	<table class="SAAListTableForm" cellpadding="0" cellspacing="0"><tr><?php
         foreach($fieldsA as $field) {
             $fieldId=$field."_".$functionId;
             ?>
			 <td> <input type="text" class="SAATextBox" value="" placeholder="<?php echo $field; ?>" id="<?php echo $fieldId; ?>" />     </td>
	<?php  }

    ?> 
	<td> <input type="button" value="Save" onclick="saa.addValue('<?php echo $functionId; ?>')" style="cursor:pointer;" /></td>
	</tr>
	</table>
</div>
...
<div id="<?php echo $popupBoxDataId ?>" class="SAA_popupBOX_data">
