  ...
  <td ><b>Status</b></td>  
    ...
    while($record=$os->mfa($rsRecords)) {
        ...
        ?>
							...
							<td> 
							<?php if($os->access('wtView')) { ?>
							<span  class="actionLink" ><a href="javascript:void(0)"  onclick="WT_photogalleryGetById('<?php echo $record['photoGalleryId'];?>')" >Edit</a></span>  <?php } ?>  </td>
								
<td>  <?php echo
    $os->rowByField('categoryName', 'gallerycatagory', 'galleryCatagoryId', $record['galleryCatagoryId']); ?></td> 
  <td><img src="<?php  echo BASE_URL.$record['name']; ?>"  height="70" width="70" /></td>  
  <td><?php echo $record['title']?> </td>  
  <td> <?php if(isset($os->photoGalleryStatus[$record['status']])) {
      echo  $os->photoGalleryStatus[$record['status']];
  } ?></td> 
...
if($os->get('WT_photogalleryEditAndSave')=='OK') {
    ...
    $name=$os->UploadPhoto('name', BASE_DIR.'wtos-images');
    if($name!='') {
...
if($os->get('WT_photogalleryGetById')=='OK') {
    ...
    if($record['name']!='') {
        $record['name']=BASE_URL.$record['name'];
    }
